...
export function logError(
  ...
  try {
    ...
    if (!config.ENABLE_DETAILED_LOGGING && config.LOG_LEVEL === 'error') {
    ...
    // Si no se puede obtener la configuración, usar logging básico
  }
  ...
  try {
    if (isAuthError(error)) {
      ...
      console.warn('⚠️ Validation Error:', JSON.stringify(logData, null, 2))
    } else if (isRecoverableError(error)) {
      console.warn('🔄 Recoverable Error:', JSON.stringify(logData, null, 2))
    } else {
      console.error('❌ Critical Error:', JSON.stringify(logData, null, 2))
    }
  } catch (logError) {
    // Fallback si el logging falla
    console.error('❌ Critical Error:', error?.message || 'Unknown error occurred')
  }
}
  ...
  for (let attempt = 0; attempt <= maxRetries; attempt++) {
    try {
      ...
      try {
        ...
        // Si no se puede obtener la configuración, continuar
      }
