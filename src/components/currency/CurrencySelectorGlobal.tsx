...
export function CurrencySelectorGlobal({ className }: { className?: string }) {
  ...
  return (
    <div className={cn('flex flex-col items-end', className)}>
      ...
      >
        <SelectTrigger className="h-9 w-[130px] sm:w-auto sm:min-w-[200px] text-xs bg-gray-800 border-gray-600 text-white hover:bg-gray-700 focus:ring-2 focus:ring-cyan-500 cursor-pointer gap-2 pl-3 pr-3">
          ...
          <span className="truncate hidden sm:inline">{currencyInfo.name} ({currency})</span>
          <span className="truncate sm:hidden">{currency}</span>
        </SelectTrigger>
