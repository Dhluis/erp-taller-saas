    ...
    const stats = {
      ...
      byStatus: {
        ...
        proposal: leads.filter(
          (l) => l.status === "proposal" || l.status === "appointment",
        ).length,
      ...
      lossRate:
        leads.length > 0
          ? (
              (leads.filter((l) => l.status === "lost").length / leads.length) *
              100
            ).toFixed(2)
          : "0.00",
      ...
      averageDaysToConvert: (() => {
        const convertedLeads = leads.filter(
          (l) => l.status === "converted" && l.converted_at,
        );
      ...
      // Top asesores
      topAssignees: (() => {
        const assigneeStats = leads.reduce(
          (acc, lead) => {
            if (lead.assigned_to) {
