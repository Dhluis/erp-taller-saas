...
export async function DELETE(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const { data: existing } = await org.admin.from('collections').select('id').eq('id', id).eq('organization_id', org.organizationId).single()
    if (!existing) return NextResponse.json({ success: false, error: 'Ingreso no encontrado' }, { status: 404 })

    const { error } = await org.admin.from('collections').delete().eq('id', id).eq('organization_id', org.organizationId)
