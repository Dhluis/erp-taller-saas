...
export async function GET(request: NextRequest) {
  try {
    ...
    if (authError || !authUser) {
      console.error(
        "[GET /api/users/me] Usuario no autenticado:",
        authError?.message,
      );
    ...
    if (!userProfile) {
      console.log(
        "[GET /api/users/me] Perfil no encontrado ni en caché ni en DB (ambas tablas), verificando autoreparación...",
      );
      ...
      try {
        ...
        if (!finalOrganizationId) {
          console.log(
            "[GET /api/users/me] Buscando vinculación por email en system_users...",
          );
          const { data: legacyUser } = await (supabaseAdmin || supabase)
            .from("system_users")
          ...
          if (legacyUser) {
            ...
            fullName =
              ...
              fullName;
            console.log(
              `✅ [GET /api/users/me] Organización encontrada vía system_users: ${finalOrganizationId}`,
            );
          }
        }
        ...
        if (!finalOrganizationId) {
          const { data: existingOrg } = await (supabaseAdmin || supabase)
            .from("organizations")
            .select("id")
            .eq("email", authUser.email || "")
            .maybeSingle();
          ...
          if (existingOrg?.id) {
            ...
            console.log(
              `✅ [GET /api/users/me] Organización encontrada vía coincidencia de email: ${finalOrganizationId}`,
            );
          }
        }
        ...
        if (!finalOrganizationId) {
          console.warn(
            `⚠️ [GET /api/users/me] Usuario ${authUser.email} no tiene organización vinculada.`,
          );
          return NextResponse.json(
            {
              error:
                "Usuario sin organización vinculada. Por favor contacta a soporte o completa tu registro.",
            },
            { status: 404 },
          );
        }
        ...
        // ✅ CRÍTICO: Usar supabase (con cookies de sesión) para la inserción
        // Si supabaseAdmin existe, usarlo; si no, usar supabase que tiene la sesión activa
        const insertClient = supabaseAdmin || supabase;
        console.log(
          `🔄 [GET /api/users/me] Creando perfil en tabla 'users' para sincronizar sesión...`,
        );
        const { data: newUser, error: createError } = (await (
          insertClient as any
        )
          .from("users")
          .insert({
        ...
        if (createError) {
          console.error(
            "[GET /api/users/me] Error sincronizando perfil:",
            createError.message,
            createError.code,
          );
          return NextResponse.json(
        ...
        await import("@/lib/database/queries/users").then((m) =>
          m.invalidateUserProfileCache(authUser.id),
        );
        ...
        console.error(
          "[GET /api/users/me] Error en proceso de autoreparación:",
          createErr.message,
        );
        return NextResponse.json(
    ...
    if (!user) {
      console.error(
        "[GET /api/users/me] Usuario es null después de query y autoreparación",
      );
      return NextResponse.json(
    ...
    if (user.organization_id) {
      ...
      if (wError) {
        console.warn(
          "[GET /api/users/me] Error obteniendo workshops (no crítico):",
          wError.message,
        );
      } else {
        workshops = workshopsData || [];
        console.log(
          `[GET /api/users/me] Workshops encontrados: ${workshops.length}`,
        );
      }
      ...
      if (sError) {
        console.warn(
          "[GET /api/users/me] Error obteniendo company_settings (no crítico):",
          sError.message,
        );
      } else {
        companySettings = settingsData;
        console.log(
          "[GET /api/users/me] Configuración de empresa obtenida:",
          companySettings ? "Encontrada" : "No encontrada",
        );
      }
    }
    ...
    if (companySettings) {
      ...
      if (!companySettings.terms_pdf_url && appDefaults.terms_pdf_url) {
        console.log(
          "[GET /api/users/me] Recuperando terms_pdf_url del backup JSONB",
        );
...
export async function PATCH(request: NextRequest) {
  try {
    ...
    const { data: updated, error } = await (supabaseAdmin as any)
      ...
      .select(
        "id, auth_user_id, email, full_name, role, phone, avatar_url, is_active, organization_id, created_at, updated_at",
      )
      .single();
    ...
    return NextResponse.json({
      profile: { ...updated, name: updated.full_name || "" },
    });
  } catch (error: any) {
