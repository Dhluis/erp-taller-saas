/**
 * EJEMPLO PRÁCTICO DE MIGRACIÓN - COTIZACIÓN DESDE ORDEN DE TRABAJO
 * 
...
export async function POST(request: NextRequest) {
  ...
  const context = createLogContext(organizationId, undefined, 'quotation-from-workorder', 'POST', {});

  try {
    validateOrganization(organizationId);
    const body = await request.json();
    logger.info('Creando cotización desde orden de trabajo', context, { workOrderId: body.work_order_id });
