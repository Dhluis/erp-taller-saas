...
export async function POST(request: NextRequest) {
  try {
    ...
    const from = {
      ...
      name: (process.env.SMTP_FROM_NAME || 'Confia Drive').trim(),
    };
    ...
    await (sgMail as any).send({
      to: testEmail,
      from,
      subject: '✅ Email de Prueba - Confia Drive',
      html: `
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px;">
          ...
          <div style="background: white; padding: 30px; border: 1px solid #e5e7eb; border-top: none; border-radius: 0 0 10px 10px;">
            ...
            <p style="font-size: 16px; color: #374151; margin-bottom: 20px;">
              Este es un email de prueba desde <strong>Confia Drive</strong>.
            </p>
          ...
          <div style="text-align: center; margin-top: 20px; padding: 20px;">
            <p style="font-size: 12px; color: #9ca3af; margin: 0;">
              Confia Drive - Sistema de Gestión para Talleres Automotrices
            </p>
          </div>
        </div>
      `,
      text: `
        ✅ Email de Prueba - Confia Drive
        
        Hola,
        
        Este es un email de prueba desde Confia Drive.
