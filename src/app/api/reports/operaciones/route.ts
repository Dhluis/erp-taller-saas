...
import { getOperationsReport } from '@/lib/database/queries/reports'
import { getTenantContext } from '@/lib/core/multi-tenant-server'
import { canViewFinancialReports, UserRole } from '@/lib/auth/permissions'
import { createClient } from '@/lib/supabase/server'

// GET /api/reports/operaciones
export async function GET(request: NextRequest) {
  try {
    ...
    const report = await getOperationsReport(organizationId, startDate, endDate)
    return NextResponse.json({ data: report, error: null })
  } catch (error: any) {
    console.error('Error in GET /api/reports/operaciones:', error)
