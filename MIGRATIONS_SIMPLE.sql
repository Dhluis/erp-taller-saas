-- Migración simplificada para Eagles System ERP
-- Ejecutar este script en el SQL Editor de Supabase

-- 1. Crear extensión UUID si no existe
CREATE EXTENSION IF NOT EXISTS "uuid-ossp";

-- 2. Crear tabla organizations PRIMERO
...
-- 8. Crear tipo ENUM para notificaciones
DO $$ BEGIN
...
-- 13. Agregar campaign_id a leads
...
-- 20. Crear políticas RLS básicas (permitir todo por ahora para testing)
DO $$ BEGIN
    CREATE POLICY "Enable all for organizations" ON public.organizations FOR ALL USING (true);
EXCEPTION
    WHEN duplicate_object THEN null;
END $$;
...
DO $$ BEGIN
    CREATE POLICY "Enable all for vehicles" ON public.vehicles FOR ALL USING (true);
EXCEPTION
    WHEN duplicate_object THEN null;
END $$;
...
-- 22. Crear función para actualizar updated_at
...
-- 23. Aplicar triggers de updated_at a todas las tablas
DO $$
DECLARE
    table_name TEXT;
BEGIN
